...
RSpec.describe CurrencyConverter do
  describe '#format' do
    it 'formats USD' do
      usd_amount = 1225.25
      result = described_class.new(usd_amount, 'USD').format
      expect(result).to eq('$1,225.25')
    end

    it 'converts and formats USD to GBP' do
      usd_amount = 1225.25
      result = described_class.new(usd_amount, 'GBP').format
      expect(result).to eq('£950.43')
    end

    it 'converts and formats USD to EUR' do
      usd_amount = 1225.25
      result = described_class.new(usd_amount, 'EUR').format
    ...
    it 'returns nil if currency unsupported' do
      usd_amount = 1225.25
      result = described_class.new(usd_amount, 'FOO').format
  ...
  describe '.format' do
    it 'is same as #format' do
      usd_amount = 1225.25
      result = described_class.format(usd_amount, 'USD')
      expect(result).to eq('$1,225.25')
    end
  end
  ...
  describe '.add' do
    it 'adds amounts and returns formatted total for given currency' do
      ...
      result = described_class.add(amounts, 'EUR')
      expect(result).to eq('€1.389,63')
    end

    it 'returns nil if currency unsupported' do
      amounts = [100.25, 500.50, 900.90]
      result = described_class.add(amounts, 'FOO')
      expect(result).to be_nil
    end
    ...
    it 'supports large floating point entries' do
      ...
      result = described_class.add(amounts, 'USD')
